...
void setup() {
   ...
   // еще тут можно (нужно) будет настроить на OUTPUT ноги которые будут соединены с индикатором...
}
...
void loop() {
  ...
  // здесь нужно будет добавить вызов метода showNumber(i / 100)
  // делить на 100 или другое число - это надо будет экспериментально выяснить,
  // желательно чтобы расстояние получалось, например, в дециметрах (можно также в пядях, вершках и т.п.)
}

// этого метода еще не было, но он будет :)
void showNumber(int number) {
   // здесь надо будет запрограммировать вывод цифры, переданной в "number" на индикатор
}
